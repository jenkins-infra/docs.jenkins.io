...
----
<1> This `image` parameter (of the xref:user-docs:pipeline:syntax.adoc#agent[`agent`]
...
* The Node container becomes the xref:user-docs:glossary:index.adoc#agent[agent] that
...
<3> Defines a xref:user-docs:pipeline:syntax.adoc#stage[`stage`] (directive) called
...
[.boxshadow]
image:tutorials:ROOT:node-js-react-11-test-stage-runs-successfully-with-output.png[alt="Test
...
+
[.boxshadow]
image:tutorials:ROOT:node-js-react-21-deliver-stage-pauses-for-user-input.png[alt="Deliver
...
xref:#run-jenkins-in-docker[above] - i.e. `--name jenkins-tutorials`).
