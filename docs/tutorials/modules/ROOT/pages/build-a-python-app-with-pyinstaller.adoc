...
*Note:* Unlike the xref:index.adoc[other tutorials] in this documentation, this tutorial
...
xref:#run-jenkins-in-docker[run Jenkins in Docker] from xref:index.adoc[another tutorial].

You can stop this tutorial at any point in time and continue from where you left
off.

If you've already run though xref:index.adoc[another tutorial], you can skip the
xref:#prerequisites[Prerequisites] and xref:#run-jenkins-in-docker[Run Jenkins in
Docker] sections below and proceed on to xref:#fork-sample-repository[forking the
...
. In the *Repository URL* field, specify the directory path of your locally
  cloned repository xref:#fork-sample-repository[above],
...
----
<1> The xref:user-docs:pipeline:syntax.adoc#agent[`agent`] section with the `none`
...
xref:user-docs:pipeline:syntax.adoc#stage[`stage`] directive must specify its own
`agent` section.
<2> Defines a xref:user-docs:pipeline:syntax.adoc#stage[`stage`] (directive) called
`Build` that appears on the Jenkins UI.
<3> This `image` parameter (of the xref:user-docs:pipeline:syntax.adoc#agent[`agent`]
...
* The Python container becomes the xref:user-docs:glossary:index.adoc#agent[agent] that
...
successfully.
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-02-initial-pipeline-runs-successfully-step-opened.png[alt="Initial
Pipeline runs successfully",width=100%]
. Click the *X* at the top-right to return to the main Blue Ocean interface.
+
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-03-main-blue-ocean-interface.png[alt="Main
...
<2> This `image` parameter (of the xref:user-docs:pipeline:syntax.adoc#agent[`agent`]
section's `docker` parameter) downloads the
...
* The pytest container becomes the xref:user-docs:glossary:index.adoc#agent[agent] that
...
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-11-test-stage-runs-successfully-with-output.png[alt="Test
...
<2> This
link:https://www.jenkins.io/doc/pipeline/tour/environment/[`environment`]
block defines two variables which will be used later in the 'Deliver' stage.
<3> This
link:https://www.jenkins.io/doc/pipeline/steps/workflow-basic-steps/#dir-change-current-directory[`dir`]
step (of the link:https://www.jenkins.io/doc/pipeline/steps/workflow-basic-steps[`basic steps`] section) creates a new subdirectory
named by the build number. The final program will be created in that directory by pyinstaller.
`BUILD_ID` is one of the pre-defined Jenkins xref:user-docs:pipeline:jenkinsfile.adoc#using-environment-variables[environment variables] and is available in all jobs.
<4> This
link:https://www.jenkins.io/doc/pipeline/steps/workflow-basic-steps/#unstash-restore-files-previously-stashed[`unstash`]
step (of the link:https://www.jenkins.io/doc/pipeline/steps/workflow-basic-steps[`basic steps`] section) restores
...
* You'll have separate Jenkins and PyInstaller (for Linux) containers running
  locally in Docker.
* The PyInstaller container becomes the xref:user-docs:glossary:index.adoc#agent[agent]
...
<6> This
link:https://www.jenkins.io/doc/pipeline/steps/core/#archiveArtifacts-archive-the-artifacts[`archiveArtifacts`]
...
xref:user-docs:pipeline:syntax.adoc#post[`post`] section's `success` condition that
...
+
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-21-deliver-stage-runs-successfully.png[alt="Deliver
...
executable file.
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-22-deliver-stage-output-only.png[alt="Deliver
stage output only",width=100%]
. Click the *X* at the top-right to return to the main Blue Ocean interface,
  which lists your previous Pipeline runs in reverse chronological order.
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-23-main-blue-ocean-interface-with-all-previous-runs-displayed.png[alt="Main
...
. From the main Blue Ocean interface, access your last Pipeline run you
  performed xref:#add-a-final-deliver-stage-to-your-pipeline[above]. To do this,
...
[.boxshadow]
image:tutorials:ROOT:python-pyinstaller-24-deliver-stage-artifacts-page.png[alt="Deliver
