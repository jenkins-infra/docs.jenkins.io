...
The exact duration depends on the speed of your machine and whether or not you've already xref:create-a-pipeline-in-blue-ocean.adoc#run-jenkins-in-docker[run Jenkins in Docker] from another tutorial.
...
If you've already run through xref:index.adoc[another tutorial], you can skip the prerequisites and xref:create-a-pipeline-in-blue-ocean.adoc#run-jenkins-in-docker[run Jenkins in Docker] sections below and proceed to xref:#fork-sample-repository[forking the sample repository].
