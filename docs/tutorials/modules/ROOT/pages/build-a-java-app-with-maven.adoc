...
The exact duration will depend on the speed of your machine and whether or not you've already xref:user-docs:installing-jenkins:_partials/_run-jenkins-in-docker.adoc#run-jenkins-in-docker[run Jenkins in Docker] from xref:index.adoc[another tutorial].

You can stop this tutorial at any time and continue from where you left off.

If you've already run through xref:index.adoc[another tutorial], you can skip the xref:#prerequisites[Prerequisites] and xref:user-docs:installing-jenkins:_partials/_run-jenkins-in-docker.adoc#run-jenkins-in-docker[run Jenkins in Docker] sections below and proceed to xref:#fork-sample-repository[forking the sample repository].
Make sure you have link:https://git-scm.com/downloads[Git] installed locally.
If you need to restart Jenkins, simply follow the restart instructions in xref:user-docs:installing-jenkins:_partials/_run-jenkins-in-docker.adoc#stopping-and-restarting-jenkins[Stopping and restarting Jenkins], and then proceed.
...
. Enter the directory path of your locally cloned repository xref:#fork-sample-repository[above] in *Script Path*.
...
----
<1> This `image` parameter of the xref:user-docs:pipeline:syntax.adoc#agent[`agent`] section's `docker` parameter downloads the link:https://hub.docker.com/_/maven/[`maven:3.9.6-eclipse-temurin-17-alpine` Docker image], if not already available on your machine, and runs this image as a separate container.
This means that:
* You'll have separate Jenkins and Maven containers running locally in Docker.
* The Maven container becomes the xref:user-docs:glossary:index.adoc#agent[agent] that Jenkins uses to run your Pipeline project.
...
NOTE: Unlike the Docker data volume created for `jenkins-data` xref:user-docs:pipeline:tour/getting-started.adoc#download-and-run-jenkins-in-docker[`above`], the Docker host's filesystem is effectively cleared out each time Docker is restarted, so you lose the downloaded Maven repository artifacts each time Docker restarts.
<3> Defines a xref:user-docs:pipeline:syntax.adoc#stage[`stage`] (directive) called `Build` that appears on the Jenkins UI.
...
+
[.boxshadow]
image:tutorials:ROOT:java-maven-01-downloading-maven-docker-image.png[alt="Downloading
...
[.boxshadow]
image:tutorials:ROOT:java-maven-02-initial-pipeline-downloading-maven-artifacts.png[alt="Downloading
Maven artifacts",width=100%]

+
The Blue Ocean interface displays green if Jenkins built your Java application successfully.
[.boxshadow]
image:tutorials:ROOT:java-maven-03-initial-pipeline-runs-successfully.png[alt="Initial
Pipeline runs successfully",width=100%]
. Select the *X* at the top-right to return to the main Blue Ocean interface.
+
[.boxshadow]
image:tutorials:ROOT:java-maven-04-main-blue-ocean-interface.png[alt="Main Blue Ocean
interface",width=100%]
...
If Docker has not restarted since you last ran the Pipeline xref:#create-your-initial-pipeline-as-a-jenkinsfile[above], then no Maven artifacts are downloaded during the "Build" stage.
...
[.boxshadow]
image:tutorials:ROOT:java-maven-11-test-stage-runs-successfully-with-output.png[alt="Test
...
+
[.boxshadow]
image:tutorials:ROOT:java-maven-21-deliver-stage-runs-successfully.png[alt="Deliver
...
[.boxshadow]
image:tutorials:ROOT:java-maven-22-deliver-stage-output-only.png[alt="Deliver stage
...
[.boxshadow]
image:tutorials:ROOT:java-maven-23-main-blue-ocean-interface-with-all-previous-runs-displayed.png[alt="Main
