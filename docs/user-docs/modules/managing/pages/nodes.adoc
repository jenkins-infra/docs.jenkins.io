...
Builds in a xref:user-docs:scaling:architecting-for-scale.adoc#distributed-builds-architecture[distributed builds architecture] use nodes, agents, and executors, which are distinct from the Jenkins controller itself.
