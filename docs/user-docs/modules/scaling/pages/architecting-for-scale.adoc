...
See
xref:user-docs:security:controller-isolation.adoc[Distributed Builds]
in the
xref:dev-docs:security:index.adoc[Securing Jenkins]
...
Learn more about Agents in xref:managing:nodes.adoc#creating-agents[Managing Nodes]
...
image::user-docs:scaling:inbound-agent.png[Inbound agent node screen, scaledwidth=90%]
...
* Hardware: see the user-docs:scaling:hardware-recommendations.adoc#hardware-recommendations[Hardware Recommendations] page
...
** The System Property xref:managing:system-properties.adoc#jenkins-model-jenkins-buildsdir[jenkins.model.Jenkins.buildsDir]
...
See the xref:system-administration:backing-up.adoc[Backup policies] for more details.
...
footnote:[These switches are used to configure out of the box link:https://www.jenkins.io/blog/2018/04/06/jenkins-essentials/[Jenkins Essentials] instances.]
...
Note that starting with Jenkins 2.119, the User Interface for this was replaced by the `jenkins.model.Jenkins.buildsDir` system property. See the xref:managing:system-properties.adoc[dedicated _Features Controlled with System Properties_ wiki page] for more details.
