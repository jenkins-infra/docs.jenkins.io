...
Now you can install Jenkins to a fresh deployment and "git clone" this _$JENKINS_HOME_ from the git repository to your new controller. 
You will need to replace the files in the new controller with your version-controlled files to
...
configurations. 
Depending on your implementation of a test controller, you will
...
script on the
xref:managing:script-console.adoc[Jenkins script console]:
...
Additional information on hardware recommendations can be be found on the xref:hardware-recommendations.adoc[Hardware Recommendations] page
