...
NOTE: 2.222.1 and 2.204.6 were released on the same day, and both contain the same security fixes. This upgrade guide only covers the upgrade from 2.204.6. See the link:/doc/upgrade-guide/2.204/[it's upgrade guide] for information related to upgrading from 2.204.5, including all information related to security fixes common to both releases.
...
Inbound TCP Agent Protocol versions 1, 2, and 3 (also known as JNLP Protocols or Java Web Start Protocols) have been removed.
These protocols have been deprecated link:/blog/2017/08/11/remoting-update/[since Jenkins 2.75 in 2017].

link:https://plugins.jenkins.io/ssh-slaves[SSH Build Agents Plugin] uses the latest `agent.jar`, but other agent types may need to have their `agent.jar` updated manually.
...
In 2017 the `Overall/RunScripts`, `Overall/UploadPlugins`, and `Overall/ConfigureUpdateCenter` were announced as potentially dangerous permissions and hidden from view in link:https://plugins.jenkins.io/matrix-auth[Matrix Authorization Plugin] and link:https://plugins.jenkins.io/role-strategy[Role-based Strategy Plugin] to prevent accidental misconfigurations (link:https://www.jenkins.io/security/advisory/2017-04-10/#matrix-authorization-strategy-plugin-allowed-configuring-dangerous-permissions[2017-04-10 security advisory]).
...
link:https://issue-redirect.jenkins.io/issue/33596[JENKINS-33596], link:https://issue-redirect.jenkins.io/issue/60913[JENKINS-60913]
...
link:https://plugins.jenkins.io/swarm[Swarm Plugin] needs to be updated to version 3.18 or above, otherwise the error described in link:https://issue-redirect.jenkins.io/issue/61029[JENKINS-61029] occurs.
This also removes the network discovery capability of the plugin.
...
Instances with plugins that depend on specific details of the Jenkins UI (like link:https://plugins.jenkins.io/simple-theme-plugin[Simple Theme plugin]) may experience issues with this release and may need to be updated.
...
link:https://plugins.jenkins.io/windows-slaves[WMI Windows Agents Plugin] needs to be updated to version 1.3.1 or newer (released in March 2017).
