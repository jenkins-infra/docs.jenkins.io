...
== Upgrading to Jenkins 2.289.1

=== New icons

The new icons introduced with Jenkins 2.289.1 replace the previous icons.
The link:https://plugins.jenkins.io/greenballs[Green Balls plugin] is not compatible with the new icons.
The link:https://plugins.jenkins.io/greenballs[Green Balls plugin] can be disabled or removed.
...
As Jenkins has developed further, capabilities moved from inside Jenkins core to dedicated Jenkins plugins, like the link:https://plugins.jenkins.io/ant[Ant plugin] and the link:https://plugins.jenkins.io/javadoc[Javadoc plugin].
...
The link:https://plugins.jenkins.io/ant[Ant plugin] and the link:https://plugins.jenkins.io/javadoc[Javadoc plugin] are no longer bundled with Jenkins.
...
In this release we detached the SSHD Jenkins core module to a separate link:https://plugins.jenkins.io/sshd[SSH Server Plugin].
...
If you link:/doc/book/managing/plugins/[Manage Jenkins Plugins] as Code (e.g. via `plugins.txt` in Docker, Helm charts or Plugin Installation Manager Tool), you might need to explicitly declare the plugin requirement after the upgrade to Jenkins 2.289.1 or above.
