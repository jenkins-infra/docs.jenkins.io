...
link:https://github.com/jenkinsci/remoting/blob/master/CHANGELOG.md#320[Remoting changelog], link:https://issue-redirect.jenkins.io/issue/50237[JENKINS-50237]
...
Without this upgrade, some remote operations on agents may fail if the call results in an exception, even if the exception could be handled.

link:https://plugins.jenkins.io/ssh-slaves[SSH Slaves] based agents will always use the latest `agent.jar`, but other agent types may need to have their `agent.jar` updated manually.
...
* link:https://plugins.jenkins.io/swarm[Swarm Plugin] Client needs to be updated to version 3.11 or above.
* Agent Docker Packages offered by the Jenkins project (link:https://hub.docker.com/r/jenkins/agent/[jenkins/agent] and link:https://hub.docker.com/r/jenkins/inbound-agent/[jenkins/inbound-agent]) need to be updated to version 3.19-1 or above.
...
link:https://issue-redirect.jenkins.io/issue/46652[JENKINS-46652], link:https://issue-redirect.jenkins.io/issue/47142[JENKINS-47142], link:https://issue-redirect.jenkins.io/issue/26143[JENKINS-26143]
