...
xref:./getting-started.adoc#defining-a-pipeline-in-scm[these instructions].
====
...
covered in more detail in the xref:handling-failure[Handling Failure] section below.
...
link:https://www.jenkins.io/doc/pipeline/steps/workflow-durable-task-step[More information on scripts].
...
xref:#secret-text[secret text], xref:#usernames-and-passwords[username and
password], as well as xref:#secret-files[secret file] credentials. If you want to
handle other types of credentials, refer to the xref:#for-other-credential-types[For other credential types] section (below).
...
When setting the credential environment variable in the xref:user-docs:pipeline:syntax.adoc#environment[`environment`] directive:
...
this Pipeline were moved immediately within the xref:user-docs:pipeline:syntax.adoc#declarative-pipeline[`pipeline`] block (as is the case in the xref:#secret-text[Secret text] Pipeline
...
[#withcredentials-script-examples]
  * To combine more than one credential in a single `withCredentials( ... )
  { ... }` Pipeline step, see xref:#combining-credentials-in-one-step[Combining
...
Groovy string interpolation can leak sensitive environment variables (i.e. credentials, see: xref:handling-credentials[Handling credentials]).
...
to accept user-specified parameters at runtime via the xref:user-docs:pipeline:syntax.adoc#parameters[parameters directive]. Configuring parameters with Scripted Pipeline is done
...
In the xref:#test[Test] example above, the `sh` step was modified to never return a
...
<2> The parameter in `agent`/`node` allows for any valid Jenkins label expression. Consult the xref:user-docs:pipeline:syntax.adoc[Pipeline Syntax] section for more details.
