...
This video reviews using the xref:pipeline:syntax.adoc#options[`buildDiscarder`] directive in individual Pipeline jobs.
...
See xref:user-docs:pipeline:cps-method-mismatches.adoc[Pipeline CPS method mismatches] for more details and some examples of things that may be problematic.
