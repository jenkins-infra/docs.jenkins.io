...
In the
xref:./running-multiple-steps.adoc[previous section]
...
* All the steps contained within the block are queued for execution by Jenkins.
  As soon as an xref:user-docs:glossary:index.adoc#executor[ executor] is available, the
  steps will begin to execute.
* A xref:user-docs:glossary:index.adoc#workspace[ workspace] is allocated which will
  contain files checked out from source control as well as any additional
  working files for the Pipeline.


There are several
xref:user-docs:pipeline:syntax.adoc#agent[ways to define agents]
...
xref:user-docs:pipeline:syntax.adoc#agent[syntax reference].
...
**xref:./environment.adoc[continue to "Using environment variables."]**
