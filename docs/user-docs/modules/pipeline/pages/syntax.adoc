...
This section builds on the information introduced in xref:pipeline:getting-started.adoc[Getting started with Pipeline] and should be treated solely as a reference.
For more information on how to use Pipeline syntax in practical examples, refer to the xref:jenkinsfile.adoc[Using a Jenkinsfile] section of this chapter.
As of version 2.5 of the Pipeline plugin, Pipeline supports two discrete syntaxes which are detailed below.
For the pros and cons of each, refer to the xref:#compare[Compare].

As discussed at the xref:pipeline:getting-started.adoc[start of this chapter], the most fundamental part of a Pipeline is the "step". Basically, steps tell Jenkins _what_ to do and serve as the basic building block for both Declarative and Scripted Pipeline syntax.

For an overview of available steps, please refer to the link:https://www.jenkins.io/doc/pipeline/steps[Pipeline Steps reference] which contains a comprehensive list of steps built into Pipeline as well as steps provided by plugins.
...
* Blocks must only consist of xref:#declarative-sections[declarative-directives], xref:#declarative-steps[Declarative Steps], or assignment statements.
* A property reference statement is treated as a no-argument method invocation.
So, for example, `input` is treated as `input()`.

You can use the xref:getting-started.adoc#directive-generator[Declarative Directive Generator] to help you get started with configuring the directives and sections in your Declarative Pipeline.
...
Sections in Declarative Pipeline typically contain one or more xref:#declarative-directives[Declarative Directives] or xref:#declarative-steps[Declarative Steps].
...
| Parameters
| xref:#parameters[Described below]
...
docker:: Execute the Pipeline, or stage, with the given container which will be dynamically provisioned on a xref:user-docs:glossary:index.adoc#node[ node] pre-configured to accept Docker-based Pipelines, or on a node matching the optionally defined `label` parameter. 
...
----
<1> Defining `agent none` at the top-level of the Pipeline ensures that xref:user-docs:glossary:index.adoc#executor[ an Executor] will not be assigned unnecessarily.
...
The `post` section defines one or more additional xref:#declarative-steps[steps] that are run upon the completion of a Pipeline's or stage's run (depending on the location of the `post` section within the Pipeline). `post` can support any of the following xref:#post-conditions[ post-condition] blocks: `always`, `changed`, `fixed`, `regression`, `aborted`, `failure`, `success`, `unstable`, `unsuccessful`, and `cleanup`.
...
<2> xref:#post-conditions[ Post-condition] blocks contain xref:#declarative-steps[ steps] the same as the xref:#steps[steps] section.
=====
...
The `steps` section defines a series of one or more xref:#declarative-steps[ steps] to be executed in a given `stage` directive.
...
The values for these user-specified parameters are made available to Pipeline steps via the `params` object, refer to the xref:#parameters-example[Parameters Example] for its specific usage.
...
The `stage` directive goes in the `stages` section and should contain a xref:#steps[steps] section, an optional `agent` section, or other stage-specific directives.
...
The `input` directive on a `stage` allows you to prompt for input, using the link:https://www.jenkins.io/doc/pipeline/steps/pipeline-input-step/#input-wait-for-interactive-input[`input` step].
...
Refer to xref:#parameters[] for more information.
...
This is the same as if the child conditions were nested in an `allOf` condition (refer to the xref:#when-example[ examples] below).
...
This section is identical to any other xref:#sequential-stages[ `stages` section].  
...
Declarative Pipelines may use all the available steps documented in the link:https://www.jenkins.io/doc/pipeline/steps[Pipeline Steps reference], which contains a comprehensive list of steps, with the addition of the steps listed below which are *only supported* in Declarative Pipeline.

==== script

The `script` step takes a block of xref:scripted-pipeline[] and executes that in the Declarative Pipeline.
...
`script` blocks of non-trivial size and/or complexity should be moved into xref:./shared-libraries.adoc[Shared Libraries] instead.
...
As discussed at the xref:pipeline:syntax.adoc[start of this chapter], the most fundamental part of a Pipeline is the "step".
...
Scripted Pipeline does *not* introduce any steps which are specific to its syntax; link:https://www.jenkins.io/doc/pipeline/steps[Pipeline Steps reference] contains a comprehensive list of steps provided by Pipeline and plugins.
...
In order to provide _durability_, which means that running Pipelines can survive a restart of the Jenkins xref:user-docs:glossary:index.adoc#controller[ controller], Scripted Pipeline must serialize data back to the controller.
...
Both are able to utilize xref:./shared-libraries.adoc[Shared Libraries]
