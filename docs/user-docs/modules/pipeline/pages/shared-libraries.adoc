...
The matching `.txt`, if present, can contain documentation, processed through the system’s configured https://wiki.jenkins.io/display/JENKINS/Markup+formatting[markup formatter] (so may really be HTML, Markdown, etc., though the `.txt` extension is required). This documentation will only be visible on the xref:pipeline:getting-started.adoc#global-variable-reference[Global Variable Reference] pages that are accessed from the navigation sidebar of Pipeline jobs that import the shared library. In addition, those jobs must run successfully once before the shared library documentation will be generated.
...
image::user-docs:pipeline:add-global-pipeline-libraries.png["Add a Global Pipeline Library", role=center]
...
These libraries may be marked “trusted“, meaning they can run any methods in Java, Groovy, Jenkins internal APIs, Jenkins plugins, or third-party libraries.
This allows you to define libraries that encapsulate individually unsafe APIs in a higher-level wrapper that is safe for use from any pipeline.
...
If you only have _Overall/Manage_ permission, you can still configure global libraries, but only “untrusted” ones that run in the Groovy sandbox just like typical Pipeline scripts.
...
image::user-docs:pipeline:configure-global-pipeline-library.png["Configuring a Global Pipeline Library", role=center]
...
image::user-docs:pipeline:global-pipeline-library-modern-scm.png["Configuring a 'Modern SCM' for a Pipeline Library", role=center]
...
image::user-docs:pipeline:global-pipeline-library-legacy-scm.png["Configuring a 'Legacy SCM' for a Pipeline Library", role=center]
...
xref:#global-shared-libraries[Global Shared Library] or as a
xref:#folder-level-shared-libraries[ Folder-level Shared Library]
