...
Pipeline is designed to easily use link:https://docs.docker.com/[Docker] images as the execution environment for a single xref:user-docs:glossary:index.adoc#stage[Stage] or the entire Pipeline.
...
Additional arguments, such as the `registryUrl`, are described in the xref:../syntax/index.adoc#agent-parameters[agent parameters] syntax documentation.
...
Pipeline supports adding custom arguments that are passed to Docker, allowing users to specify custom link:https://docs.docker.com/engine/tutorials/dockervolumes/[Docker Volumes] to mount, which can be used for caching data on the xref:user-docs:glossary:index.adoc#agent[agent] between Pipeline runs.
...
In contrast to the xref:#execution-environment[previous approach] of using an "off-the-shelf" container, using the `agent { dockerfile true }` syntax builds a new image from a `Dockerfile`, rather than pulling one from link:https://hub.docker.com[Docker Hub].
...
The `agent { dockerfile true }` syntax supports a number of other options, which are described in more detail in the xref:user-docs:pipeline:syntax.adoc#agent[Pipeline Syntax] section.
...
By default, Pipeline assumes that _any_ configured xref:user-docs:glossary:index.adoc#agent[agent] is capable of running Docker-based Pipelines.
...
Pipeline provides a global option on the *Manage Jenkins* page and on the xref:user-docs:glossary:index.adoc#folder[Folder] level, for specifying which agents (by xref:user-docs:glossary:index.adoc#label[Label]) to use for running Docker-based Pipelines.

image::user-docs:pipeline:configure-docker-label.png[Configuring the Pipeline Docker Label]
...
One "sidecar" running MySQL, and another providing the xref:#execution-environment[execution environment] by using the Docker link:https://docs.docker.com/engine/userguide/networking/default_network/dockerlinks/[container links].
...
The return value can also be used to publish the Docker image to link:https://hub.docker.com[Docker Hub] or a xref:#custom-registry[ custom Registry], via the `push()` method, for example:
