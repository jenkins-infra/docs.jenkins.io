:page-aliases: user-docs:system-administration:configuring-content-security-policy.adoc
...
* *Are some agents not fully trusted?* Even when Agent To Controller Access is used to limit what agents can do on the
...
Changes to the system property will be effective immediately, so it's possible to set this system property temporarily via the xref:managing:script-console.adoc[Script Console], allowing you to experiment with different values:
