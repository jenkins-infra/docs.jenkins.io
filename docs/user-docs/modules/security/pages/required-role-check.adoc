...
In those cases, it is still recommended to update the plugin as described xref:dev-docs:security:remoting-callables.adoc[in the developer documentation].
====
...
xref:dev-docs:security:remoting-callables.adoc[See the developer documentation] for considerations what constitutes unsafe `Callable` behavior.
====
...
xref:dev-docs:security:remoting-callables.adoc[See the developer documentation] for advice how to change plugins to be compatible with this change while not creating a security vulnerability.
