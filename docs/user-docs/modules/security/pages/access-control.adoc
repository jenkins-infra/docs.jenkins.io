...
See xref:user-docs:security:permissions.adoc#administer[the documentation on the access given to administrators] about the impact of unintentionally granting Administer permission.
...
Built-in node::
Users with limited permissions xref:user-docs:security:controller-isolation.adoc[must not be able to configure jobs that run on the built-in node].
...
//See xref:dev-docs:security:access-control/permissions.adoc#overall-read[the documentation of the level of access that granting basic read access gives].
...
With this permission, users can upload and install plugins and have access to the xref:managing:script-console.adoc[Script Console].
...
For more details about the various permissions in Jenkins and the level of access they grant, see xref:dev-docs:security:index.adoc[Security].

== Disabling Access Control

See xref:dev-docs:security:index.adoc[Security].
