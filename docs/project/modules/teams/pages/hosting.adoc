...
. Subscribe to the xref:community:mailing-lists:index.adoc#jenkinsci-dev-googlegroups-com[Jenkins Developer Mailing list]
...
. Join the xref:community:chat:index.adoc[`#jenkins-hosting` IRC channel] and xref:community:chat:index.adoc#cloaks[obtain a cloak].
  It will be needed for ChatOps.
. Request voice permission (`+vV`) from a channel operator in the xref:community:chat:index.adoc[`#jenkins-hosting` IRC channel] to get access to the bot commands.
...
To join it, follow xref:community:chat:index.adoc[these guidelines].
...
* xref:projects:infrastructure:ircbot.adoc[Jenkins IRC Bot] - ChatOps tool for managing the hosting and ownership requests.
