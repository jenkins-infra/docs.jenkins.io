...
====
See xref:releasing-cd.adoc[setting up automated plugin release] instead of this guide.
====

== Prerequisites

. Make sure you have permissions to release the plugin. xref:requesting-hosting.adoc[See this guide to learn more details]

== Artifactory Credentials for Maven

You will need to tell Maven your credentials to access xref:artifact-repository.adoc[Artifactory].
...
// Not sure about this:
// NOTE: While it is the possible to specify the username and password on the command line, that would require your accounts on GitHub and the Jenkins community to match, and prevent you from using two-factor authentication on GitHub.
// Neither is a recommend practice.

== Troubleshooting

First, make sure your plugin uses a reasonably up-to-date xref:plugin-development:updating-parent.adoc[parent POM].
