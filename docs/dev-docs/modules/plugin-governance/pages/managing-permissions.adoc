...
* **Release Permissions** which are needed to deploy releases to the xref:publishing:artifact-repository.adoc[Jenkins artifact repository].
  Snapshots can be deployed by contributors without this permission.

NOTE: If you want to adopt a plugin, please see xref:adopt-a-plugin.adoc[Adopt a Plugin] instead.
...
. Via a request in the xref:community:mailing-lists:index.adoc[developer mailing list]
. Via the ChatOps command in the `#jenkins` IRC channel, if you have voice permissions there (see the xref:projects:infrastructure:ircbot.adoc[IRCBot])
...
If there no response from a maintainer, a xref:adopt-a-plugin.adoc[Plugin Adoption Process] might be used to request ownership of the plugin.

There are two following ways to do that:

. Via ChatOps command in the `#jenkins` IRC channel, if you have voice permissions there (see the xref:projects:infrastructure:ircbot.adoc[IRCBot])
. Via a request in the xref:community:mailing-lists:index.adoc[developer mailing list].
...
Release Permissions are needed to deploy releases to the xref:publishing:artifact-repository.adoc[Jenkins artifact repository].
...
Permission removal can be requested in the same way, e.g. during xref:adopt-a-plugin.adoc[plugin adoption].
...
* xref:plugin-governance:adopt-a-plugin.adoc[Adopting a plugin]
****