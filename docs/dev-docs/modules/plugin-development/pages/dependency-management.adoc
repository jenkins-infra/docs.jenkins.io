...
* xref:#jenkins-core-bom[Jenkins core Bill of Materials (BOM)] centrally defines the **library versions** provided by Jenkins core
* xref:#jenkins-plugin-bom[Plugin Bill of Materials (BOM)] centrally defines a set of **plugin versions** that have been tested together
* xref:#dependabot[Dependabot] automatically proposes a pull request when a new version of a dependency is released
...
See xref:plugin-development:updating-parent.adoc#understanding-requireupperbounddeps-failures-and-fixes[Understanding requireUpperBoundDeps failures and fixes] for more information.
...
. Confirm that you are using the xref:plugin-development:updating-parent.adoc[current parent pom] with the command `mvn versions:update-parent`
...
.References
****
* link:https://github.com/jenkinsci/plugin-pom/[Plugin parent POM 4.0 and up]
* link:https://github.com/jenkinsci/jenkins/blob/master/bom/pom.xml[Jenkins Core BOM]
****