...
<4> This is a https://commons.apache.org/proper/commons-jexl/[JEXL] expression. `it` refers to the Java object the view belongs to (or similar to `this` in Java), in this case the `HelloWorldAction` instance. `it.name` is equivalent to a getter call (`getName()`).
