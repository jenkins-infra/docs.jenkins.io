...
To prevent further security vulnerabilities like link:https://www.jenkins.io/security/advisory/2021-10-06/#SECURITY-2499[SECURITY-2499] from having an impact on Jenkins users, the method has been redefined to return plain text in Jenkins 2.315 and LTS 2.303.2, and its output is no longer rendered as HTML on the UI.
