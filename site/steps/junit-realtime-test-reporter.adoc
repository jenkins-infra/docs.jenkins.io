...
++++
<a href="https://plugins.jenkins.io/junit-realtime-test-reporter">Visit the Jenkins Plugins site</a>
++++
...
++++
<div><div>
 Runs a block and archives JUnit results at the end, exactly like the <code>junit</code> step would. The difference is that test results are displayed as they appear.
</div></div>
...
<li><code>parseInterval : long</code> (optional)
<div><div>
 <p>The interval between two file system scans to parse test results (in seconds). Must be a positive value greater than 0. Default value is calculated as 1/100 of the estimated build duration, but never more often than every 5 seconds.</p>
</div></div>
