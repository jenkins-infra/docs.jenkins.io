...
- id: SECURITY-3030
  ...
  description: |-
    ...
    If that happens, these limits can be changed by setting the Java system properties xref:user-docs:managing:system-properties.adoc#hudson-util-multipartformdataparser-fileupload_max_files[`hudson.util.MultipartFormDataParser.FILEUPLOAD_MAX_FILES`] and xref:user-docs:managing:system-properties.adoc#org-kohsuke-stapler-requestimpl-fileupload_max_files[`org.kohsuke.stapler.RequestImpl.FILEUPLOAD_MAX_FILES`] to a bigger value, or to `-1` to completely disable them.

    [NOTE]
    ====
    These releases of Jenkins also introduce additional Java system properties that can be set to restrict request sizes:

    * xref:user-docs:managing:system-properties.adoc#hudson-util-multipartformdataparser-fileupload_max_file_size[`hudson.util.MultipartFormDataParser.FILEUPLOAD_MAX_FILE_SIZE`] and xref:user-docs:managing:system-properties.adoc#org-kohsuke-stapler-requestimpl-fileupload_max_file_size[`org.kohsuke.stapler.RequestImpl.FILEUPLOAD_MAX_FILE_SIZE`] allow limiting the size (in bytes) of individual fields that can be processed in one `multipart/form-data` request.
    * xref:user-docs:managing:system-properties.adoc#hudson-util-multipartformdataparser-fileupload_max_size[`hudson.util.MultipartFormDataParser.FILEUPLOAD_MAX_SIZE`] and xref:user-docs:managing:system-properties.adoc#org-kohsuke-stapler-requestimpl-fileupload_max_size[`org.kohsuke.stapler.RequestImpl.FILEUPLOAD_MAX_SIZE`] allow limiting the total request size (in bytes) that can be processed in one `multipart/form-data` request.
