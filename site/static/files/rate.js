...
function do_loaddata() {
    ...
    for (var anchors = document.getElementsByTagName("H3"), i = 0; i < anchors.length; i++) {
        ...
        txt =
            (r && r[0] ? r[0] + " " : "0 ") +
            health("sunny", r && r[0] ? "" : "light", v, 1, "No major issues with this release") +
            (r && r[1] ? r[1] + " " : "0 ") +
            health("cloudy", r && r[1] ? "" : "light", v, 0, "I experienced notable issues") +
            (r && r[2] ? r[2] + " " : "0 ") +
            health("storm", r && r[2] ? "" : "light", v, -1, "I had to roll back");
        if (r && r.length > 3) {
            txt += '<span class="related-issues">Community reported issues: ';
            var issues = [];
            for (j = 3; j < r.length; j += 2) {
                issues.push({ id: r[j], count: r[j + 1] });
            }
            ...
            for (j = 0; j < issues.length; j++)
                txt +=
                    issues[j].count +
                    '&times;<a href="https://issues.jenkins.io/browse/JENKINS-' +
                    issues[j].id +
                    '">JENKINS-' +
                    issues[j].id +
                    "</a> ";
...
function rate(version, rating) {
    var issue =
        rating <= 0 ? prompt("Please provide issue number from our JIRA causing trouble:", "") : "";
    ...
    script.type = "text/javascript";
    script.src =
        "https://rating.jenkins.io/rate/submit.php?version=" +
